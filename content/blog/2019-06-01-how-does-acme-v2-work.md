...
Over the last few weeks, I've been setting up this website on Azure. It's a simple static website, so the setup is just to upload the static assets to an Azure Storage Account and set up an Azure CDN with a custom domain name in front of it. I also wanted to use my own custom TLS certificate for it sourced from [Let's Encrypt,](https://letsencrypt.org) which means I needed to set up an automatic renewal workflow for said cert. But the web server is in Azure CDN's control, so I can't run something turnkey like [Certbot,](https://certbot.eff.org/) and I wanted to do this as much as possible by myself rather than relying on third-party stuff anyway, so I implemented my own ACME renewal workflow and run it in an Azure Function.

I'll write more details about the Azure setup later. For now, I want to share what I learned by providing a simple explanation of how an ACME v2 client implementation works.
...
All interactions with the server other than the directory request and the "new nonce" request are authenticated. The client generates an account key in one of the formats supported by the server based on the JSON Web Key RFC, and uses this to sign its requests using the JSON Web Signature RFC.
...
The server responds to the directory request with a `200 OK` response. This response is a JSON object that maps URL types to URLs. The minimal URLs the client needs from this response are:

- The "new account" URL, with key `newAccount`
- The "new nonce" URL, with key `newNonce`
- The "new order" URL, with key `newOrder`

See [section 7.1.1 Directory of the ACME RFC](https://ietf-wg-acme.github.io/acme/draft-ietf-acme-acme.html#rfc.section.7.1.1) for details.
...
It is possible for the server to return a `Replay-Nonce` header in the response of the directory request itself. In the general case, the server does not do so. In this case, the client gets an initial nonce by sending an HTTP HEAD request to the "new nonce" URL. The client expects the server to respond with a `200 OK` response that definitely contains a `Replay-Nonce` header.

{{% /section %}}


{{% section %}}
{{% h 2 "The JWS request body format" %}}

All interactions with the server other than the directory request and the "new nonce" request are HTTP POST requests, and contain a request body that is constructed using the JWS format. This is a multi-step process based on the JSON Web Signature RFC linked above.

The client starts with the payload that it wants to send. This might be an empty payload, or it might be an object. In the latter case, the client serializes the object to a JSON string, then encodes the string using the "Base 64 Encoding with URL and Filename Safe Alphabet" described in [section 5 of RFC 4648.](https://tools.ietf.org/html/rfc4648#section-5) This is now the "encoded payload".

It is important to note that the payload can be empty, which means the "encoded payload" is the empty string. The HTTP request still has a request body, since this empty "encoded payload" is still wrapped in a JWS envelope as described below. Specifically, the payload is empty in what the ACME RFC calls "POST-as-GET" requests, which refers to requests that get the current status of an object like a REST GET request would, but are nevertheless sent as POST requests with a JWS body. See [section 6.3 GET and POST-as-GET Requests of the ACME RFC](https://ietf-wg-acme.github.io/acme/draft-ietf-acme-acme.html#rfc.section.6.3) for more details.

Note that the URL-safe base64 encoding is the only kind of encoding used in the entire specification. Future references to base64 encoding in this document will refer to this same URL-safe encoding.
...
The client expects the server to return a `201 Created` or `200 OK` response. The former implies that the server has created a new account corresponding to the account key, while the latter indicates the server has already seen this account key before and so will use the existing account. The body of the response contains a JSON object representing the account. Of particular importance is the `status` field of this object, which must be the value `"valid"`. The response is described in full detail in [section 7.1.2 Account Objects of the ACME RFC.](https://ietf-wg-acme.github.io/acme/draft-ietf-acme-acme.html#rfc.section.7.1.2)

The response will contain a `Location` header that contains the "account URL". This account URL uniquely identifies the account, and will be used for future requests, thus the client must save it in its state.

As mentioned above, the server returns a `200 OK` response if it already has an existing account corresponding to the account key. In this way, the "new account" URL also functions as a "get existing account" URL. Thus if the client wants to reuse the same account key multiple times, it can use this to "discover" the account URL of the existing account. Alternatively, if the client expects the account to have already been created previously, it can persist the account key *and* the account URL, and skip this step entirely.

Since the client now has the account URL, future requests use the second format of the protected header that contains the `kid` key.
...
The client expects the server to return a `201 Created` response.

The response contains a `Location` header that contains the "order URL". The client should save this URL in its order state.

The body of this response contains a JSON object representing the order. If the order for these identifiers already existed and that order is still valid, then the server returns the existing order. The response status code is still `201 Created` in this case.

- The value of the `authorizations` key of this object is an array of strings, each of which represents an "authorization URL" for this order. There will be one such URL for each identifier in the order request.

- The value of the `finalize` key of this object is a string representing the "finalize URL" of this order.

- The value of the `status` key of this object represents the state of the order, and is used to determine how to proceed.
...
The client performs the next set of steps for each authorization URL in the order.


{{% section %}}
{{% h 3 "Completing an authorization" %}}

The client fetches the authorization object by performing a POST-as-GET request to the authorization URL, and expects the server to return a `200 OK` response. The response contains a JSON object that represents the current state of the authorization.
...
- The `challenges` value of the authorization object is an array of challenge objects.
	...
	- The `url` value is the URL of the challenge. The client uses this URL to indicate to the server that it has satisfied the requirements of the challenge, so the server should begin its verification.
...
- If the authorization is in the `"pending"` state, it is waiting for the client to complete at least one challenge of the order.
...
To complete a pending authorization, the client chooses one of its challenges and tries to complete it.
...
The client sends an HTTP POST request to the order's "finalize URL" with a payload that looks like this:
...
The client expects an `200 OK` response from the server. It then polls the order URL until the order has reached the `"valid"` state.
...
If the order is in the `"valid"` state, the order has been completed. The order object will also have a `certificate` key, whose value is a string representing the URL of the signed certificate. The client downloads the certificate from this URL and ends the order workflow. The client combines this certificate with the private key of the CSR it had generated previously, and begins using it for its webserver.
...
I hope this serves as a useful starting point for anyone wanting to implement an ACME v2 client from scratch. Of course, there are far more details in the ACME RFC that I haven't covered here, so check the RFC if something doesn't work the way I describe it here.
