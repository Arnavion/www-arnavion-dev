...
(An HN user pointed out [here](https://news.ycombinator.com/item?id=21567568) that DDG has a `!man` bang command - it forwards to [manpages.me](https://manpage.me) However this site is very slow, and defaults to a newer version of FreeBSD, so I don't use it.)
...
FreeBSD's manual for `gawk` is at [this URL,](https://docs.freebsd.org/info/gawk/gawk.info.Index.html) and is much more explanatory than the default `awk` manual.
