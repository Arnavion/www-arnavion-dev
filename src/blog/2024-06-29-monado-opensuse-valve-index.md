...
Fortunately Monado v24 has [a new driver](https://old.reddit.com/r/virtualreality_linux/comments/14cmh2c/) for tracking the SteamVR base stations that actually just uses the SteamVR driver itself, which has much better tracking. The Monado build does not enable this driver by default unless the build environment has a `~/.steam` directory, which would obviously not be the case for a distro package builder, so I had to configure the build to explicitly enable the driver. This change is only in [my OBS repository's Monado package](https://build.opensuse.org/package/show/home:Arnavion/monado) for now; it has complications for upstreaming to `hardware:xr` that I will discuss later. Note that the `LH_DRIVER=steamvr` env var mentioned in the Reddit post is outdated and will not work with Monado v24; the correct env var is `STEAMVR_LH_ENABLE=true`.
...
Earlier I mentioned Monado supports the SteamVR lighthouse driver and I have only enabled it in my OBS repository's `monado` package and haven't sr'd it to `hardware:xr`. That is for a similar reason; the situation with that is that the `monado-service` binary compiled for OpenSUSE is being asked to load a SteamVR library (`~/.local/share/Steam/steamapps/common/SteamVR/drivers/lighthouse/bin/linux64/driver_lighthouse.so`) that is presumably compiled for the Steam Runtime. It happens to work, but it is possible that the `hardware:xr` package maintainer will (understandably) not be happy with doing that in the `monado` package itself. I'll check later.
